...
export function useExternalFileDrag(scope: string) {
  ...
  const unified = available && capabilities.data?.unifiedTrackDrag === true
  ...
  const begin = async (ids: string[], includeTrackIds = false) => {
    ...
    try {
      const result = await bridge.dragFiles(ids, includeTrackIds)
      explain(result.reason === 'internal-only-missing-files'
        ? result.dropAccepted
          ? 'Track selection dropped inside WISP. Some audio files are missing; external file dragging needs every selected file. Check the playlist or mix for the result.'
          : 'Some selected audio files are missing. You can drop this selection into a WISP playlist or mix, but not another app. Reconnect the drive or relink missing files for external dragging.'
        : result.dropAccepted
        ? includeTrackIds
          ? 'Track selection dropped. Check the playlist, mix or destination app for the result.'
  ...
  return { available, unified, busy, begin, feedback: feedback?.scope === scope ? feedback : null, explain, dismiss: () => setFeedback(null),
