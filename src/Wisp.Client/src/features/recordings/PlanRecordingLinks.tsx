...
export function PlanRecordingLinks({ planId }: { planId: string }) {
  const origin = useQuery({ queryKey: ['plan-recording-origin', planId], queryFn: async () => (await apiGet<{ recordingId: string; recordingTitle: string; recordingExists: boolean } | null>(`/api/recording-feedback/plans/${planId}/origin`)) ?? null })
  ...
  return <div className="max-h-40 shrink-0 overflow-auto border-b border-[var(--color-border)] px-6 py-2 text-sm">
    ...
    {origin.data?.recordingId && <p className="text-xs">Revised from {origin.data.recordingTitle}. {origin.data.recordingExists ? <button className="min-h-11 underline" onClick={() => { useRecordingNavigation.getState().select(origin.data!.recordingId); useCurrentPage.getState().setPage('recordings') }}>Open source recording</button> : 'Source recording removed; lineage preserved.'}</p>}
